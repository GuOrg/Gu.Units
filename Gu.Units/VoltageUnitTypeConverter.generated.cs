namespace Gu.Units
{
    ...
    public class VoltageUnitTypeConverter : TypeConverter
    {
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            ...
            if (destinationType == typeof(InstanceDescriptor) && value is VoltageUnit)
            {
                MethodInfo method = typeof(VoltageUnit).GetMethod(nameof(VoltageUnit.Parse), new Type[] { typeof(string) });
                if (method != null)
                {
                    var args = new object[] { ((VoltageUnit)value).Symbol };
