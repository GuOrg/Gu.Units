...
[assembly: TypeForwardedTo(typeof(KinematicViscosity))]
[assembly: TypeForwardedTo(typeof(KinematicViscosityUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class KinematicViscosityConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is KinematicViscosity) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(KinematicViscosity)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return kinematicViscosity.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<KinematicViscosityUnit>.TryParse(format, out this.quantityFormat))
            {
