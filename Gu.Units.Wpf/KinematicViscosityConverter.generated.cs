namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="KinematicViscosity"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class KinematicViscosityConverter : MarkupExtension, IValueConverter
    {
        private KinematicViscosityUnit? unit;
        private Binding binding;
        private QuantityFormat<KinematicViscosityUnit> quantityFormat;
        private QuantityFormat<KinematicViscosityUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.KinematicViscosityConverter"/>.
        /// </summary>
        public KinematicViscosityConverter()
        {
        }
        ...
        public KinematicViscosityConverter(KinematicViscosityUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="KinematicViscosityUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(KinematicViscosityUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<KinematicViscosityUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<KinematicViscosityUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<KinematicViscosityUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<KinematicViscosityUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    ...
                    this.errorText.AppendLine(StringFormatParser<KinematicViscosityUnit>.CreateFormatErrorString(value, typeof(KinematicViscosityUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is KinematicViscosity))
            {
                message += $"{GetType().Name} only supports converting from {typeof(KinematicViscosity)}";
            }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(KinematicViscosity) || targetType == typeof(KinematicViscosity?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new KinematicViscosity((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new KinematicViscosity(d, this.unit.Value);
                        }
                        KinematicViscosity result;
                        if (KinematicViscosity.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<KinematicViscosityUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<KinematicViscosityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<KinematicViscosityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
