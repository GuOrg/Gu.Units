namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="SpecificEnergy"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class SpecificEnergyConverter : MarkupExtension, IValueConverter
    {
        private SpecificEnergyUnit? unit;
        private Binding binding;
        private QuantityFormat<SpecificEnergyUnit> quantityFormat;
        private QuantityFormat<SpecificEnergyUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.SpecificEnergyConverter"/>.
        /// </summary>
        public SpecificEnergyConverter()
        {
        }
        ...
        /// <summary>
        /// Gets and sets the <see cref="SpecificEnergyUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(SpecificEnergyUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<SpecificEnergyUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<SpecificEnergyUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<SpecificEnergyUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<SpecificEnergyUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    ...
                    this.errorText.AppendLine(StringFormatParser<SpecificEnergyUnit>.CreateFormatErrorString(value, typeof(SpecificEnergyUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is SpecificEnergy))
            {
                message += $"{GetType().Name} only supports converting from {typeof(SpecificEnergy)}";
            }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return specificEnergy.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return specificEnergy.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(SpecificEnergy) || targetType == typeof(SpecificEnergy?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new SpecificEnergy((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new SpecificEnergy(d, this.unit.Value);
                        }
                        SpecificEnergy result;
                        if (SpecificEnergy.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<SpecificEnergyUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<SpecificEnergyUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<SpecificEnergyUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
