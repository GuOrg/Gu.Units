...
[assembly: TypeForwardedTo(typeof(SpecificEnergy))]
[assembly: TypeForwardedTo(typeof(SpecificEnergyUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class SpecificEnergyConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is SpecificEnergy) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(SpecificEnergy)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return specificEnergy.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<SpecificEnergyUnit>.TryParse(format, out this.quantityFormat))
            {
