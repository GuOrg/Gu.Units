...
[assembly: TypeForwardedTo(typeof(ElectricalConductance))]
[assembly: TypeForwardedTo(typeof(ElectricalConductanceUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class ElectricalConductanceConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is ElectricalConductance) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(ElectricalConductance)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return electricalConductance.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        {
            if (this.provideValueTarget == null)
            {
                this.bindingStringFormat = null;
                return;
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<ElectricalConductanceUnit>.TryParse(format, out this.quantityFormat))
            {
