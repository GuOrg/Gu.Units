namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="LuminousIntensity"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class LuminousIntensityConverter : MarkupExtension, IValueConverter
    {
        private LuminousIntensityUnit? unit;
        private Binding binding;
        private QuantityFormat<LuminousIntensityUnit> quantityFormat;
        private QuantityFormat<LuminousIntensityUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of the <see cref="Gu.Units.Wpf.LuminousIntensityConverter"/> class.
        /// </summary>
        public LuminousIntensityConverter()
        {
        }
        ...
        /// <summary>
        /// Gets or sets the <see cref="LuminousIntensityUnit"/>
        /// </summary>
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<LuminousIntensityUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<LuminousIntensityUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<LuminousIntensityUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<LuminousIntensityUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<LuminousIntensityUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<LuminousIntensityUnit>.CreateFormatErrorString(value, typeof(LuminousIntensityUnit)));
                }
            }
        }
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value != null && !(value is LuminousIntensity))
            {
                message += $"{GetType().Name} only supports converting from {typeof(LuminousIntensity)}";
            }
        ...
        public object ConvertBack(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(LuminousIntensity) || targetType == typeof(LuminousIntensity?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new LuminousIntensity((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new LuminousIntensity(d, this.unit.Value);
                        }
                        LuminousIntensity result;
                        if (LuminousIntensity.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<LuminousIntensityUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<LuminousIntensityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<LuminousIntensityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
