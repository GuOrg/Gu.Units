...
[assembly: TypeForwardedTo(typeof(LuminousIntensity))]
[assembly: TypeForwardedTo(typeof(LuminousIntensityUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class LuminousIntensityConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is LuminousIntensity) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(LuminousIntensity)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return luminousIntensity.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<LuminousIntensityUnit>.TryParse(format, out this.quantityFormat))
            {
