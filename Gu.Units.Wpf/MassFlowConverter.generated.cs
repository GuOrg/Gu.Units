...
namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="MassFlow"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class MassFlowConverter : MarkupExtension, IValueConverter
    {
        private MassFlowUnit? unit;
        private Binding binding;
        private QuantityFormat<MassFlowUnit> quantityFormat;
        private QuantityFormat<MassFlowUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.MassFlowConverter"/>.
        /// </summary>
        ...
        /// <summary>
        /// Gets and sets the <see cref="MassFlowUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(MassFlowUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<MassFlowUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<MassFlowUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<MassFlowUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<MassFlowUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<MassFlowUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<MassFlowUnit>.CreateFormatErrorString(value, typeof(MassFlowUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is MassFlow))
            {
                message += $"{GetType().Name} only supports converting from {typeof(MassFlow)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return massFlow.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return massFlow.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return massFlow.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(MassFlow) || targetType == typeof(MassFlow?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new MassFlow((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new MassFlow(d, this.unit.Value);
                        }
                        MassFlow result;
                        if (MassFlow.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<MassFlowUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<MassFlowUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<MassFlowUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
