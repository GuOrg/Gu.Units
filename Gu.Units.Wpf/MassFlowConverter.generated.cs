...
[assembly: TypeForwardedTo(typeof(MassFlow))]
[assembly: TypeForwardedTo(typeof(MassFlowUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class MassFlowConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is MassFlow) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(MassFlow)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return massFlow.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<MassFlowUnit>.TryParse(format, out this.quantityFormat))
            {
