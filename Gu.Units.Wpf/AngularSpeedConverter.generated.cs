namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="AngularSpeed"/>
    /// </summary>
    ...
    public class AngularSpeedConverter : MarkupExtension, IValueConverter
    {
        ...
        private QuantityFormat<AngularSpeedUnit> quantityFormat;
        private QuantityFormat<AngularSpeedUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of the <see cref="Gu.Units.Wpf.AngularSpeedConverter"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="AngularSpeedUnit"/>
        /// </summary>
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<AngularSpeedUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<AngularSpeedUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<AngularSpeedUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<AngularSpeedUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<AngularSpeedUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<AngularSpeedUnit>.CreateFormatErrorString(value, typeof(AngularSpeedUnit)));
                }
            }
        }
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value != null && !(value is AngularSpeed))
            {
                message += $"{GetType().Name} only supports converting from {typeof(AngularSpeed)}";
            }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return angularSpeed.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<AngularSpeedUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<AngularSpeedUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<AngularSpeedUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
