namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="MolarHeatCapacity"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class MolarHeatCapacityConverter : MarkupExtension, IValueConverter
    {
        private MolarHeatCapacityUnit? unit;
        private Binding binding;
        private QuantityFormat<MolarHeatCapacityUnit> quantityFormat;
        private QuantityFormat<MolarHeatCapacityUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.MolarHeatCapacityConverter"/>.
        /// </summary>
        public MolarHeatCapacityConverter()
        {
        }
        ...
        public MolarHeatCapacityConverter(MolarHeatCapacityUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="MolarHeatCapacityUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(MolarHeatCapacityUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<MolarHeatCapacityUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<MolarHeatCapacityUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<MolarHeatCapacityUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<MolarHeatCapacityUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<MolarHeatCapacityUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<MolarHeatCapacityUnit>.CreateFormatErrorString(value, typeof(MolarHeatCapacityUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is MolarHeatCapacity))
            {
                message += $"{GetType().Name} only supports converting from {typeof(MolarHeatCapacity)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return molarHeatCapacity.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return molarHeatCapacity.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return molarHeatCapacity.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(MolarHeatCapacity) || targetType == typeof(MolarHeatCapacity?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new MolarHeatCapacity((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new MolarHeatCapacity(d, this.unit.Value);
                        }
                        MolarHeatCapacity result;
                        if (MolarHeatCapacity.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<MolarHeatCapacityUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<MolarHeatCapacityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<MolarHeatCapacityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
