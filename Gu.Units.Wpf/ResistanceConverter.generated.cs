...
[assembly: TypeForwardedTo(typeof(Resistance))]
[assembly: TypeForwardedTo(typeof(ResistanceUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class ResistanceConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Resistance) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Resistance)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return resistance.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<ResistanceUnit>.TryParse(format, out this.quantityFormat))
            {
