namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Resistance"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class ResistanceConverter : MarkupExtension, IValueConverter
    {
        private ResistanceUnit? unit;
        private Binding binding;
        private QuantityFormat<ResistanceUnit> quantityFormat;
        private QuantityFormat<ResistanceUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.ResistanceConverter"/>.
        /// </summary>
        ...
        /// <summary>
        /// Gets and sets the <see cref="ResistanceUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(ResistanceUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<ResistanceUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<ResistanceUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<ResistanceUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<ResistanceUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<ResistanceUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<ResistanceUnit>.CreateFormatErrorString(value, typeof(ResistanceUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Resistance))
            {
                message += $"{GetType().Name} only supports converting from {typeof(Resistance)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return resistance.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return resistance.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return resistance.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<ResistanceUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<ResistanceUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<ResistanceUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
