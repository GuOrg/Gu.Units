...
[assembly: TypeForwardedTo(typeof(Illuminance))]
[assembly: TypeForwardedTo(typeof(IlluminanceUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class IlluminanceConverter : MarkupExtension, IValueConverter
    {
        ...
        [ConstructorArgument("unit"), TypeConverter(typeof(IlluminanceUnitTypeConverter))]
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Illuminance) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Illuminance)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return illuminance.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<IlluminanceUnit>.TryParse(format, out this.quantityFormat))
            {
