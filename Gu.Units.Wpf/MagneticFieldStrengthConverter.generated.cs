...
namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="MagneticFieldStrength"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class MagneticFieldStrengthConverter : MarkupExtension, IValueConverter
    {
        private MagneticFieldStrengthUnit? unit;
        private Binding binding;
        private QuantityFormat<MagneticFieldStrengthUnit> quantityFormat;
        private QuantityFormat<MagneticFieldStrengthUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.MagneticFieldStrengthConverter"/>.
        /// </summary>
        public MagneticFieldStrengthConverter()
        {
        }
        ...
        public MagneticFieldStrengthConverter(MagneticFieldStrengthUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="MagneticFieldStrengthUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(MagneticFieldStrengthUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<MagneticFieldStrengthUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<MagneticFieldStrengthUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<MagneticFieldStrengthUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<MagneticFieldStrengthUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    ...
                    this.errorText.AppendLine(StringFormatParser<MagneticFieldStrengthUnit>.CreateFormatErrorString(value, typeof(MagneticFieldStrengthUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is MagneticFieldStrength))
            {
                message += $"{GetType().Name} only supports converting from {typeof(MagneticFieldStrength)}";
            }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(MagneticFieldStrength) || targetType == typeof(MagneticFieldStrength?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new MagneticFieldStrength((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new MagneticFieldStrength(d, this.unit.Value);
                        }
                        MagneticFieldStrength result;
                        if (MagneticFieldStrength.TryParse(text, NumberStyles.Float, culture, out result))
                        {
