...
[assembly: TypeForwardedTo(typeof(MagneticFieldStrength))]
[assembly: TypeForwardedTo(typeof(MagneticFieldStrengthUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class MagneticFieldStrengthConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is MagneticFieldStrength) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(MagneticFieldStrength)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return magneticFieldStrength.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<MagneticFieldStrengthUnit>.TryParse(format, out this.quantityFormat))
            {
