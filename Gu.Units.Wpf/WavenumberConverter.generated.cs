namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Wavenumber"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class WavenumberConverter : MarkupExtension, IValueConverter
    {
        private WavenumberUnit? unit;
        private Binding binding;
        private QuantityFormat<WavenumberUnit> quantityFormat;
        private QuantityFormat<WavenumberUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.WavenumberConverter"/>.
        /// </summary>
        public WavenumberConverter()
        {
        }
        ...
        public WavenumberConverter(WavenumberUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="WavenumberUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(WavenumberUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<WavenumberUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<WavenumberUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<WavenumberUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<WavenumberUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<WavenumberUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<WavenumberUnit>.CreateFormatErrorString(value, typeof(WavenumberUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Wavenumber))
            {
                message += $"{GetType().Name} only supports converting from {typeof(Wavenumber)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return wavenumber.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return wavenumber.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return wavenumber.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(Wavenumber) || targetType == typeof(Wavenumber?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new Wavenumber((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new Wavenumber(d, this.unit.Value);
                        }
                        Wavenumber result;
                        if (Wavenumber.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<WavenumberUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<WavenumberUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<WavenumberUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
