...
[assembly: TypeForwardedTo(typeof(Wavenumber))]
[assembly: TypeForwardedTo(typeof(WavenumberUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class WavenumberConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Wavenumber) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Wavenumber)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return wavenumber.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<WavenumberUnit>.TryParse(format, out this.quantityFormat))
            {
