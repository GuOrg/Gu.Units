namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Jerk"/>
    /// </summary>
    ...
    public class JerkConverter : MarkupExtension, IValueConverter
    {
        ...
        private QuantityFormat<JerkUnit> bindingQuantityFormat;
        ...
        /// <summary>
        /// Gets and sets the <see cref="JerkUnit"/>
        /// </summary>
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<JerkUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<JerkUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<JerkUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<JerkUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<JerkUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<JerkUnit>.CreateFormatErrorString(value, typeof(JerkUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Jerk))
            {
                message += $"{GetType().Name} only supports converting from {typeof(Jerk)}";
            }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<JerkUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<JerkUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<JerkUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
