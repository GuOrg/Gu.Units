...
[assembly: TypeForwardedTo(typeof(Voltage))]
[assembly: TypeForwardedTo(typeof(VoltageUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class VoltageConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Voltage) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Voltage)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return voltage.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<VoltageUnit>.TryParse(format, out this.quantityFormat))
            {
