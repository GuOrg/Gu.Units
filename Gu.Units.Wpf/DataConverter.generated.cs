...
[assembly: TypeForwardedTo(typeof(DataUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class DataConverter : MarkupExtension, IValueConverter
    {
        ...
        public DataConverter([TypeConverter(typeof(DataUnitTypeConverter))]DataUnit unit)
        {
            Unit = unit;
        }

        [ConstructorArgument("unit"), TypeConverter(typeof(DataUnitTypeConverter))]
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Data) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Data)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return data.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<DataUnit>.TryParse(format, out this.quantityFormat))
            {
