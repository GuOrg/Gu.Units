...
[assembly: TypeForwardedTo(typeof(LengthPerUnitless))]
[assembly: TypeForwardedTo(typeof(LengthPerUnitlessUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class LengthPerUnitlessConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is LengthPerUnitless) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(LengthPerUnitless)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return lengthPerUnitless.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<LengthPerUnitlessUnit>.TryParse(format, out this.quantityFormat))
            {
