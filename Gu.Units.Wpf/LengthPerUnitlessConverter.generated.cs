namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="LengthPerUnitless"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class LengthPerUnitlessConverter : MarkupExtension, IValueConverter
    {
        private LengthPerUnitlessUnit? unit;
        private Binding binding;
        private QuantityFormat<LengthPerUnitlessUnit> quantityFormat;
        private QuantityFormat<LengthPerUnitlessUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.LengthPerUnitlessConverter"/>.
        /// </summary>
        public LengthPerUnitlessConverter()
        {
        }
        ...
        /// <summary>
        /// Gets and sets the <see cref="LengthPerUnitlessUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(LengthPerUnitlessUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<LengthPerUnitlessUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<LengthPerUnitlessUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<LengthPerUnitlessUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<LengthPerUnitlessUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<LengthPerUnitlessUnit>.CreateFormatErrorString(value, typeof(LengthPerUnitlessUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            {
                message += $"{GetType().Name} only supports converting from {typeof(LengthPerUnitless)}";
            }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(LengthPerUnitless) || targetType == typeof(LengthPerUnitless?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new LengthPerUnitless((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new LengthPerUnitless(d, this.unit.Value);
                        }
                        LengthPerUnitless result;
                        if (LengthPerUnitless.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<LengthPerUnitlessUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<LengthPerUnitlessUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
