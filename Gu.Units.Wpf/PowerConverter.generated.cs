...
namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Power"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class PowerConverter : MarkupExtension, IValueConverter
    {
        private PowerUnit? unit;
        private Binding binding;
        private QuantityFormat<PowerUnit> quantityFormat;
        private QuantityFormat<PowerUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.PowerConverter"/>.
        /// </summary>
        ...
        /// <summary>
        /// Gets and sets the <see cref="PowerUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(PowerUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<PowerUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<PowerUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<PowerUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<PowerUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<PowerUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<PowerUnit>.CreateFormatErrorString(value, typeof(PowerUnit)));
                }
            }
        }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<PowerUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
