...
[assembly: TypeForwardedTo(typeof(Power))]
[assembly: TypeForwardedTo(typeof(PowerUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class PowerConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Power) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Power)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return power.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<PowerUnit>.TryParse(format, out this.quantityFormat))
            {
