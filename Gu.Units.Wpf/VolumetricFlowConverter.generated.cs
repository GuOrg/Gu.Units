namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="VolumetricFlow"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class VolumetricFlowConverter : MarkupExtension, IValueConverter
    {
        private VolumetricFlowUnit? unit;
        private Binding binding;
        private QuantityFormat<VolumetricFlowUnit> quantityFormat;
        private QuantityFormat<VolumetricFlowUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.VolumetricFlowConverter"/>.
        /// </summary>
        public VolumetricFlowConverter()
        {
        }
        ...
        /// <summary>
        /// Gets and sets the <see cref="VolumetricFlowUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(VolumetricFlowUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<VolumetricFlowUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<VolumetricFlowUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<VolumetricFlowUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<VolumetricFlowUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<VolumetricFlowUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<VolumetricFlowUnit>.CreateFormatErrorString(value, typeof(VolumetricFlowUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is VolumetricFlow))
            {
                message += $"{GetType().Name} only supports converting from {typeof(VolumetricFlow)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return volumetricFlow.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return volumetricFlow.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return volumetricFlow.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(VolumetricFlow) || targetType == typeof(VolumetricFlow?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new VolumetricFlow((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new VolumetricFlow(d, this.unit.Value);
                        }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<VolumetricFlowUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<VolumetricFlowUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<VolumetricFlowUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
