namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="LuminousFlux"/>
    /// </summary>
    ...
    public class LuminousFluxConverter : MarkupExtension, IValueConverter
    {
        ...
        private QuantityFormat<LuminousFluxUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of the <see cref="Gu.Units.Wpf.LuminousFluxConverter"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="LuminousFluxUnit"/>
        /// </summary>
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<LuminousFluxUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<LuminousFluxUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<LuminousFluxUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<LuminousFluxUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<LuminousFluxUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<LuminousFluxUnit>.CreateFormatErrorString(value, typeof(LuminousFluxUnit)));
                }
            }
        }
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value != null && !(value is LuminousFlux))
            {
                message += $"{GetType().Name} only supports converting from {typeof(LuminousFlux)}";
            }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<LuminousFluxUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<LuminousFluxUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<LuminousFluxUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
