...
namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Conductivity"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class ConductivityConverter : MarkupExtension, IValueConverter
    {
        private ConductivityUnit? unit;
        private Binding? binding;
        private QuantityFormat<ConductivityUnit>? quantityFormat;
        private QuantityFormat<ConductivityUnit>? bindingQuantityFormat;
        private string? valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of the <see cref="Gu.Units.Wpf.ConductivityConverter"/> class.
        /// </summary>
        public ConductivityConverter()
        {
        }
        ...
        public ConductivityConverter(ConductivityUnit unit)
        {
            this.Unit = unit;
        }

        /// <summary>
        /// Gets or sets the <see cref="ConductivityUnit"/>
        /// </summary>
        ...
        public string? ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<ConductivityUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<ConductivityUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<ConductivityUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string? StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<ConductivityUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<ConductivityUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<ConductivityUnit>.CreateFormatErrorString(value, typeof(ConductivityUnit)));
                }
            }
        }
        ...
        public object? Convert(object value, Type targetType, object? parameter, CultureInfo culture)
        {
            ...
            if (value != null && !(value is Conductivity))
            {
                message += $"{this.GetType().Name} only supports converting from {typeof(Conductivity)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return conductivity.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return conductivity.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return conductivity.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<ConductivityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<ConductivityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput is null)
                            {
