...
[assembly: TypeForwardedTo(typeof(SpecificVolume))]
[assembly: TypeForwardedTo(typeof(SpecificVolumeUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class SpecificVolumeConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is SpecificVolume) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(SpecificVolume)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return specificVolume.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<SpecificVolumeUnit>.TryParse(format, out this.quantityFormat))
            {
