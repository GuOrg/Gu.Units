...
namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="SpecificVolume"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class SpecificVolumeConverter : MarkupExtension, IValueConverter
    {
        private SpecificVolumeUnit? unit;
        private Binding binding;
        private QuantityFormat<SpecificVolumeUnit> quantityFormat;
        private QuantityFormat<SpecificVolumeUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.SpecificVolumeConverter"/>.
        /// </summary>
        public SpecificVolumeConverter()
        {
        }
        ...
        public SpecificVolumeConverter(SpecificVolumeUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="SpecificVolumeUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(SpecificVolumeUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<SpecificVolumeUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<SpecificVolumeUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<SpecificVolumeUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<SpecificVolumeUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<SpecificVolumeUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<SpecificVolumeUnit>.CreateFormatErrorString(value, typeof(SpecificVolumeUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is SpecificVolume))
            {
                message += $"{GetType().Name} only supports converting from {typeof(SpecificVolume)}";
            }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(SpecificVolume) || targetType == typeof(SpecificVolume?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new SpecificVolume((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new SpecificVolume(d, this.unit.Value);
                        }
                        SpecificVolume result;
                        if (SpecificVolume.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<SpecificVolumeUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<SpecificVolumeUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<SpecificVolumeUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
