namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="CatalyticActivity"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class CatalyticActivityConverter : MarkupExtension, IValueConverter
    {
        private CatalyticActivityUnit? unit;
        private Binding binding;
        private QuantityFormat<CatalyticActivityUnit> quantityFormat;
        private QuantityFormat<CatalyticActivityUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.CatalyticActivityConverter"/>.
        /// </summary>
        public CatalyticActivityConverter()
        {
        }
        ...
        public CatalyticActivityConverter(CatalyticActivityUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="CatalyticActivityUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(CatalyticActivityUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<CatalyticActivityUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<CatalyticActivityUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<CatalyticActivityUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<CatalyticActivityUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<CatalyticActivityUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<CatalyticActivityUnit>.CreateFormatErrorString(value, typeof(CatalyticActivityUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is CatalyticActivity))
            {
                message += $"{GetType().Name} only supports converting from {typeof(CatalyticActivity)}";
            }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<CatalyticActivityUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<CatalyticActivityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<CatalyticActivityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
