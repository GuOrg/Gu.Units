...
[assembly: TypeForwardedTo(typeof(CatalyticActivity))]
[assembly: TypeForwardedTo(typeof(CatalyticActivityUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class CatalyticActivityConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is CatalyticActivity) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(CatalyticActivity)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return catalyticActivity.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<CatalyticActivityUnit>.TryParse(format, out this.quantityFormat))
            {
