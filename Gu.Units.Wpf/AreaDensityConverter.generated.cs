namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="AreaDensity"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class AreaDensityConverter : MarkupExtension, IValueConverter
    {
        private AreaDensityUnit? unit;
        private Binding binding;
        private QuantityFormat<AreaDensityUnit> quantityFormat;
        private QuantityFormat<AreaDensityUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.AreaDensityConverter"/>.
        /// </summary>
        public AreaDensityConverter()
        {
        }
        ...
        /// <summary>
        /// Gets and sets the <see cref="AreaDensityUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(AreaDensityUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<AreaDensityUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<AreaDensityUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<AreaDensityUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<AreaDensityUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<AreaDensityUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<AreaDensityUnit>.CreateFormatErrorString(value, typeof(AreaDensityUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is AreaDensity))
            {
                message += $"{GetType().Name} only supports converting from {typeof(AreaDensity)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return areaDensity.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return areaDensity.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return areaDensity.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(AreaDensity) || targetType == typeof(AreaDensity?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new AreaDensity((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new AreaDensity(d, this.unit.Value);
                        }
                        AreaDensity result;
                        if (AreaDensity.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<AreaDensityUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<AreaDensityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<AreaDensityUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
