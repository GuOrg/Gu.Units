...
[assembly: TypeForwardedTo(typeof(AreaDensity))]
[assembly: TypeForwardedTo(typeof(AreaDensityUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class AreaDensityConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is AreaDensity) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(AreaDensity)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return areaDensity.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<AreaDensityUnit>.TryParse(format, out this.quantityFormat))
            {
