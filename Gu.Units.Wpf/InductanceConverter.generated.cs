...
[assembly: TypeForwardedTo(typeof(Inductance))]
[assembly: TypeForwardedTo(typeof(InductanceUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class InductanceConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Inductance) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Inductance)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return inductance.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<InductanceUnit>.TryParse(format, out this.quantityFormat))
            {
