namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Inductance"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class InductanceConverter : MarkupExtension, IValueConverter
    {
        private InductanceUnit? unit;
        private Binding binding;
        private QuantityFormat<InductanceUnit> quantityFormat;
        private QuantityFormat<InductanceUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.InductanceConverter"/>.
        /// </summary>
        ...
        /// <summary>
        /// Gets and sets the <see cref="InductanceUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(InductanceUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<InductanceUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<InductanceUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<InductanceUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<InductanceUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    ...
                    this.errorText.AppendLine(StringFormatParser<InductanceUnit>.CreateFormatErrorString(value, typeof(InductanceUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Inductance))
            {
                message += $"{GetType().Name} only supports converting from {typeof(Inductance)}";
            }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<InductanceUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<InductanceUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<InductanceUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
