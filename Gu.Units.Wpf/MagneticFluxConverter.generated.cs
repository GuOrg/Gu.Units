...
[assembly: TypeForwardedTo(typeof(MagneticFlux))]
[assembly: TypeForwardedTo(typeof(MagneticFluxUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class MagneticFluxConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is MagneticFlux) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(MagneticFlux)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return magneticFlux.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<MagneticFluxUnit>.TryParse(format, out this.quantityFormat))
            {
