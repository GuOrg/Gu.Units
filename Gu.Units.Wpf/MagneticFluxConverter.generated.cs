...
namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="MagneticFlux"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class MagneticFluxConverter : MarkupExtension, IValueConverter
    {
        private MagneticFluxUnit? unit;
        private Binding binding;
        private QuantityFormat<MagneticFluxUnit> quantityFormat;
        private QuantityFormat<MagneticFluxUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.MagneticFluxConverter"/>.
        /// </summary>
        ...
        public MagneticFluxConverter(MagneticFluxUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="MagneticFluxUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(MagneticFluxUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<MagneticFluxUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<MagneticFluxUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<MagneticFluxUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<MagneticFluxUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<MagneticFluxUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<MagneticFluxUnit>.CreateFormatErrorString(value, typeof(MagneticFluxUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is MagneticFlux))
            {
                message += $"{GetType().Name} only supports converting from {typeof(MagneticFlux)}";
            }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(MagneticFlux) || targetType == typeof(MagneticFlux?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        MagneticFlux result;
                        if (MagneticFlux.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<MagneticFluxUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<MagneticFluxUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<MagneticFluxUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
