...
[assembly: TypeForwardedTo(typeof(Pressure))]
[assembly: TypeForwardedTo(typeof(PressureUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class PressureConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Pressure) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Pressure)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return pressure.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<PressureUnit>.TryParse(format, out this.quantityFormat))
            {
