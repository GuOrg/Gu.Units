...
[assembly: TypeForwardedTo(typeof(VolumeUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class VolumeConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Volume) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Volume)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return volume.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<VolumeUnit>.TryParse(format, out this.quantityFormat))
            {
