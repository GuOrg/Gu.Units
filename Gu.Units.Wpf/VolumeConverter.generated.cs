namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Volume"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class VolumeConverter : MarkupExtension, IValueConverter
    {
        private VolumeUnit? unit;
        private Binding binding;
        private QuantityFormat<VolumeUnit> quantityFormat;
        private QuantityFormat<VolumeUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.VolumeConverter"/>.
        /// </summary>
        ...
        /// <summary>
        /// Gets and sets the <see cref="VolumeUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(VolumeUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<VolumeUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<VolumeUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<VolumeUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<VolumeUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<VolumeUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<VolumeUnit>.CreateFormatErrorString(value, typeof(VolumeUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Volume))
            {
                message += $"{GetType().Name} only supports converting from {typeof(Volume)}";
            }
            ...
            if (this.bindingQuantityFormat != null)
            {
                ...
                {
                    return volume.GetValue(this.unit.Value);
                }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return volume.ToString(this.quantityFormat, culture);
                }

                if (this.ValueFormat != null)
                {
                    return volume.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(Volume) || targetType == typeof(Volume?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new Volume((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new Volume(d, this.unit.Value);
                        }
                        Volume result;
                        if (Volume.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<VolumeUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<VolumeUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<VolumeUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
