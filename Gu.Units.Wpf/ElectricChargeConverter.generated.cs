namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="ElectricCharge"/>
    /// </summary>
    ...
    public class ElectricChargeConverter : MarkupExtension, IValueConverter
    {
        ...
        private QuantityFormat<ElectricChargeUnit> quantityFormat;
        private QuantityFormat<ElectricChargeUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of the <see cref="Gu.Units.Wpf.ElectricChargeConverter"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="ElectricChargeUnit"/>
        /// </summary>
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<ElectricChargeUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<ElectricChargeUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<ElectricChargeUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<ElectricChargeUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<ElectricChargeUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<ElectricChargeUnit>.CreateFormatErrorString(value, typeof(ElectricChargeUnit)));
                }
            }
        }
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value != null && !(value is ElectricCharge))
            {
                message += $"{GetType().Name} only supports converting from {typeof(ElectricCharge)}";
            }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<ElectricChargeUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<ElectricChargeUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<ElectricChargeUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
