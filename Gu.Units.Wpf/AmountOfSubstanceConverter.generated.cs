...
[assembly: TypeForwardedTo(typeof(AmountOfSubstance))]
[assembly: TypeForwardedTo(typeof(AmountOfSubstanceUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class AmountOfSubstanceConverter : MarkupExtension, IValueConverter
    {
        ...
        public AmountOfSubstanceConverter([TypeConverter(typeof(AmountOfSubstanceUnitTypeConverter))]AmountOfSubstanceUnit unit)
        {
            Unit = unit;
        }

        [ConstructorArgument("unit"), TypeConverter(typeof(AmountOfSubstanceUnitTypeConverter))]
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is AmountOfSubstance) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(AmountOfSubstance)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return amountOfSubstance.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<AmountOfSubstanceUnit>.TryParse(format, out this.quantityFormat))
            {
