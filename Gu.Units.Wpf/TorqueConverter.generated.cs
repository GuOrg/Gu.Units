...
[assembly: TypeForwardedTo(typeof(Torque))]
[assembly: TypeForwardedTo(typeof(TorqueUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class TorqueConverter : MarkupExtension, IValueConverter
    {
        ...
        public TorqueConverter([TypeConverter(typeof(TorqueUnitTypeConverter))]TorqueUnit unit)
        {
            Unit = unit;
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Torque) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Torque)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return torque.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<TorqueUnit>.TryParse(format, out this.quantityFormat))
            {
