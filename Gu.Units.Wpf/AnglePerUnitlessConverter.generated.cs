namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="AnglePerUnitless"/>
    /// </summary>
    ...
    public class AnglePerUnitlessConverter : MarkupExtension, IValueConverter
    {
        ...
        private QuantityFormat<AnglePerUnitlessUnit> quantityFormat;
        private QuantityFormat<AnglePerUnitlessUnit> bindingQuantityFormat;
        private string valueFormat;
        private bool initialized;
        private StringBuilder errorText = new StringBuilder();

        /// <summary>
        /// Initializes a new instance of the <see cref="Gu.Units.Wpf.AnglePerUnitlessConverter"/> class.
        /// </summary>
        ...
        public AnglePerUnitlessConverter(AnglePerUnitlessUnit unit)
        {
            this.Unit = unit;
        }

        /// <summary>
        /// Gets or sets the <see cref="AnglePerUnitlessUnit"/>
        /// </summary>
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<AnglePerUnitlessUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<AnglePerUnitlessUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<AnglePerUnitlessUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                if (StringFormatParser<AnglePerUnitlessUnit>.TryParse(value, out this.quantityFormat))
                {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<AnglePerUnitlessUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<AnglePerUnitlessUnit>.CreateFormatErrorString(value, typeof(AnglePerUnitlessUnit)));
                }
            }
        }
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value != null && !(value is AnglePerUnitless))
            {
                message += $"{GetType().Name} only supports converting from {typeof(AnglePerUnitless)}";
            }
            ...
            if (targetType == typeof(string) || targetType == typeof(object))
            {
                ...
                {
                    return anglePerUnitless.GetValue(this.unit.Value).ToString(this.valueFormat, culture);
                }
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<AnglePerUnitlessUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<AnglePerUnitlessUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<AnglePerUnitlessUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
