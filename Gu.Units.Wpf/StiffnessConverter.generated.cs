...
[assembly: TypeForwardedTo(typeof(Stiffness))]
[assembly: TypeForwardedTo(typeof(StiffnessUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class StiffnessConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Stiffness) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Stiffness)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return stiffness.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<StiffnessUnit>.TryParse(format, out this.quantityFormat))
            {
