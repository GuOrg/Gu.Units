namespace Gu.Units.Wpf
{
    ...
    /// <summary>
    /// An <see cref="IValueConverter"/> for quantities of type <see cref="Speed"/>
    /// </summary>
    [MarkupExtensionReturnType(typeof(IValueConverter))]
    public class SpeedConverter : MarkupExtension, IValueConverter
    {
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="Gu.Units.Wpf.SpeedConverter"/>.
        /// </summary>
        ...
        public SpeedConverter(SpeedUnit unit)
        {
            Unit = unit;
        }

        /// <summary>
        /// Gets and sets the <see cref="SpeedUnit"/>
        /// </summary>
        [ConstructorArgument("unit"), TypeConverter(typeof(SpeedUnitTypeConverter))]
        ...
        public string ValueFormat
        {
            ...
            set
            {
                if (!StringFormatParser<SpeedUnit>.CanParseValueFormat(value))
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<SpeedUnit>.VerifyValueFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<SpeedUnit>.CreateFormatErrorString(value, typeof(double)));
                }
        ...
        public string StringFormat
        {
            ...
            set
            {
                ...
                else
                {
                    if (Is.DesignMode)
                    {
                        StringFormatParser<SpeedUnit>.VerifyQuantityFormat(value);
                    }

                    this.errorText.AppendLine(StringFormatParser<SpeedUnit>.CreateFormatErrorString(value, typeof(SpeedUnit)));
                }
            }
        }
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Speed))
            {
                message += $"{GetType().Name} only supports converting from {typeof(Speed)}";
            }
            ...
            CultureInfo culture)
        {
            ...
            var message = this.errorText.ToString();
            if (!(targetType == typeof(Speed) || targetType == typeof(Speed?)))
            {
                message += $"{GetType().Name} does not support converting to {targetType.Name}";
            }
            ...
            if (value is double)
            {
                return new Speed((double)value, this.unit.Value);
            }
                ...
                case Wpf.UnitInput.ScalarOnly:
                    {
                        ...
                        {
                            return new Speed(d, this.unit.Value);
                        }
                        Speed result;
                        if (Speed.TryParse(text, NumberStyles.Float, culture, out result))
                        {
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    this.quantityFormat = FormatCache<SpeedUnit>.GetOrCreate(ValueFormat, this.unit.Value, SymbolFormat.Value);
                }
            }
            ...
            if (this.UnitInput == Wpf.UnitInput.SymbolRequired)
            {
                if (string.IsNullOrEmpty(this.quantityFormat?.SymbolFormat))
                {
                    if (string.IsNullOrEmpty(this.bindingQuantityFormat?.SymbolFormat))
                    {
                        ...
                        {
                            this.quantityFormat = FormatCache<SpeedUnit>.GetOrCreate(this.ValueFormat, this.unit.Value, this.SymbolFormat.Value);
                        ...
                        {
                            this.quantityFormat = FormatCache<SpeedUnit>.GetOrCreate(this.ValueFormat, this.unit.Value);
                            if (this.UnitInput == null)
                            {
