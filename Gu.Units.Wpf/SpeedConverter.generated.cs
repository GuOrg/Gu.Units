...
[assembly: TypeForwardedTo(typeof(Speed))]
[assembly: TypeForwardedTo(typeof(SpeedUnit))]
...
namespace Gu.Units.Wpf
{
    ...
    public class SpeedConverter : MarkupExtension, IValueConverter
    {
            ...
            CultureInfo culture)
        {
            ...
            if (value != null && !(value is Speed) && Is.DesignMode)
            {
                var message = $"{GetType().Name} only supports converting from {typeof(Speed)}";
                throw new ArgumentException(message, nameof(value));
            }
            ...
            if ((SymbolFormat != null || UnitInput == Wpf.UnitInput.SymbolRequired) &&
               (targetType == typeof(string) || targetType == typeof(object)))
            {
                return speed.ToString(Unit.Value, SymbolFormat ?? Units.SymbolFormat.FractionSuperScript, culture);
            }
        ...
        private void OnStringFormatChanged()
        {
            ...
            var format = this.stringFormat == StringFormatNotSet
                ? this.bindingStringFormat
                : this.stringFormat;
            if (StringFormatParser<SpeedUnit>.TryParse(format, out this.quantityFormat))
            {
